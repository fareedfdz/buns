...
const TEXTUREMAP = { src: 'https://images.unsplash.com/photo-1550547660-d9450f859349?auto=format&fit=crop&q=80&w=1920' };
const DEPTHMAP = { src: 'https://images.unsplash.com/photo-1550547660-d9450f859349?auto=format&fit=crop&q=80&w=1920' };
...
export const Html = ({ title = 'Build Your Dreams', subtitle = 'AI-powered creativity for the next generation.', onOrder, onViewMenu }: { title?: string, subtitle?: string, onOrder?: () => void, onViewMenu?: () => void }) => {
      ...
      <div className="h-svh uppercase items-center w-full absolute z-60 pointer-events-none px-10 flex justify-center flex-col">
        ...
        <div 
          ...
          style={{ 
            ...
            transitionDelay: `${titleWords.length * 0.13 + 0.5 + subtitleDelay}s`
          }}
        >
          {onOrder && (
            <button 
              onClick={onOrder}
              className="bg-brand text-white px-8 py-4 rounded-full font-display text-xl uppercase tracking-widest hover:bg-white hover:text-brand transition-all"
            >
              Order Now
            </button>
          )}
          {onViewMenu && (
            <button 
              onClick={onViewMenu}
              className="glass text-white px-8 py-4 rounded-full font-display text-xl uppercase tracking-widest hover:bg-white/20 transition-all"
            >
              View Menu
            </button>
          )}
